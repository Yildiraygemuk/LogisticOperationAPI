...
using LogisticCompany.Helper.AppSetting;
...
namespace LogisticCompany.Core.Middleware
{
    public class JwtMiddleware
    {
        ...
        private static void AttachAccountToContext(HttpContext context, string token)
        {
            try
            {
                ...
                context.Items["LogisticCompanyUser"] = new AuthUserDto()
                {
                    UserId = Convert.ToInt32(jwtToken.Claims.First(x => x.Type == CustomClaimTypes.UserId).Value),
